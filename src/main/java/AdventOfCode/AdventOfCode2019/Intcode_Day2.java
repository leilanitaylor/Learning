...
Part 1:
    ...
    means the program is finished and should immediately halt. Encountering an unknown
    ...
    For example, suppose you have the following program:
        1,9,10,3,2,3,10,0,99,30,40,50
    For the purposes of illustration, here is the same program split into multiple lines:
        1,9,10,3,
        2,3,11,0,
        99,
        30,40,50
    The first four integers, 1,9,10,3 are the positions 0, 1, 2, and 3. Together, they
    ...
    the previous, but it multiplies instead of adding. The inputs are at positions 3 and
    11; these positions contain 70 and 50 respectively. Multiplying these produces 3500;
    ...
    Here are the initial and final states of a few more small programs:
        ...
        - 2,4,5,99,0 becomes 2,4,5,99,9801 (99*99=9801).
...
public class Intcode_Day2 {
    ...
    protected static void runOpcode(List<Integer> opcodes, int currentIndex) {
        ...
        if (opcode == 1) {
            solution = addInputs(input1, input2);
            setSolution(opcodes, solutionIndex, solution);
        }
        else if (opcode == 2) {
            solution = multInputs(input1, input2);
            setSolution(opcodes, solutionIndex, solution);
        }
        else
            System.out.println("Program has encountered and unknown opcode, something is wrong!");
    }

    protected static void setSolution(List<Integer> opcodes, int solutionIndex, int solution) {
        opcodes.set(solutionIndex, solution);
    }

    protected static int multInputs(int input1, int input2) {
        int solution = input1 * input2;

        return solution;
    }

    protected static int addInputs(int input1, int input2) {
        int solution = input1 + input2;
    ...
    private static List<Integer> readFile() throws FileNotFoundException {
        Path path = Paths.get("src/main/resources").resolve("2019_Day2_InputFile.txt");
        File opcodesFile = path.toFile();
        List<Integer> opcodes = new ArrayList<>();

        Scanner sc = new Scanner(opcodesFile);
        while (sc.hasNext()) {
            String opcodeString = sc.next();
            String[] inputNumber = opcodeString.split(",");
            for (int i = 0; i < inputNumber.length; i++)
                opcodes.add(Integer.parseInt(inputNumber[i]));
        }
