...
Part 2:
    During second Go/No Go poll, the Elf in charge of the Rocket Equation Double-Checker
    stops the launch sequence. Apparently, you forgot to include additional fuel for the
    fuel you just added.
    ...
    So, for each module mass, calculate it fuel and add it to the total. Then, treat the
    ...
    until a fuel requirement is zero or negative. For example:
        ...
        - At first, a module of mass 1969 requires 654 fuel. Then, this fuel requires 216
          ...
          required for a module of mass 1969 is 654 + 216 + 70 + 5 = 966.
        - The fuel required by a module of mass 100756 and its fuel is: 33583 + 11192 +
          3728 + 1240 + 411 + 135 + 43 + 12 + 2 = 50346.

    What is the sum of the fuel requirements for all the modules on your spacecraft when
    also taking into account the mass of the added fuel? (Calculate the fuel
...
public class FuelRequirements_Day1 {

    public static void main(String[] args) throws FileNotFoundException {
        List<Integer> modules = readFile();

        int totalFuelRequired = findTotal(modules);

        System.out.println("Total Fuel Required: " + totalFuelRequired);
    }

    protected static int findTotal(List<Integer> modules) {
        int totalFuelRequired = 0;

        for (int i =0; i < modules.size(); i++) {
            totalFuelRequired += fuelForCurrModule(modules.get(i));
        }
    ...
    private static List<Integer> readFile() throws FileNotFoundException {
        Path path = Paths.get("src/main/resources").resolve("2019_Day1_InputFile.txt");
        ...
        while (sc.hasNextLine()) {
            ...
            modules.add(Integer.parseInt(module));
        }
        return modules;
    }
}
