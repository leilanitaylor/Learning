...
class Closest2Test {
    ...
    @Test
    void testSample1convertPtsToSingleArray() {
        int[] x = {0, 3};
        int[] y = {0, 4};
        Closest2.Point[] expected = new Closest2.Point[2];
        expected[0] = new Closest2.Point(0, 0);
        expected[1] = new Closest2.Point(3, 4);
        Closest2.Point[] result = ct.convertPtsToSingleArray(x, y);
        Assertions.assertNotNull(result);
        Assertions.assertEquals(expected.length, result.length);
        for (int i = 0; i < result.length; i++) {
            Assertions.assertEquals(expected[i].x, result[i].x);
            Assertions.assertEquals(expected[i].y, result[i].y);
        }
    }

    @Test
    void testSample2convertPtsToSingleArray() {
        int[] x = {7, 1, 4, 7};
        int[] y = {7, 100, 8, 7};
        Closest2.Point[] expected = new Closest2.Point[4];
        expected[0] = new Closest2.Point(7, 7);
        expected[1] = new Closest2.Point(1, 100);
    ...
    @Test
    void testSample3ConvertPtsToSingleArray() {
        int[] x = {4, -2, -3, -1, 2, -4, 1, -1, 3, -4, -2};
        int[] y = {4, -2, -4, 3, 3, 0, 1, -1, -1, 2, 4};
        Closest2.Point[] expected = new Closest2.Point[11];
        expected[0] = new Closest2.Point(4, 4);
        expected[1] = new Closest2.Point(-2, -2);
        expected[2] = new Closest2.Point(-3, -4);
        expected[3] = new Closest2.Point(-1, 3);
        expected[4] = new Closest2.Point(2, 3);
        expected[5] = new Closest2.Point(-4, 0);
        expected[6] = new Closest2.Point(1, 1);
        expected[7] = new Closest2.Point(-1, -1);
        expected[8] = new Closest2.Point(3, -1);
        expected[9] = new Closest2.Point(-4, 2);
